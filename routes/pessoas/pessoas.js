...
export function pessoas(request, response) {
    ...
    let newbody_impala = "";
      if (cpftel.substr(0,1) == "+"){
        let ddd = cpftel.substr(3,2);
        let telefone = cpftel.substr(5);
        const newbody_impala1 = body1_impala.replace(/DDDDDDDDD/gi, ddd);
        newbody_impala = newbody_impala1.replace(/TELEFONETELEFONE/gi, telefone);
      } else {
        newbody_impala = body1_impala.replace(/XXXXXXXXXXX/gi, cpftel);
      }
    let resp = [];
    ...
    fetch("https://boavista-dados.ciasc.sc.gov.br/notebook/api/execute/impala", {
      ...
      .then((data) => {
        ...
        const body_fetch = readFileSync("./routes/pessoas/body_fetch.json");
        const body1_fetch = body_fetch.toString();
        const newbody_fetch = body1_fetch.replace(/XXXXXXXXXXX/gi, cpftel);
